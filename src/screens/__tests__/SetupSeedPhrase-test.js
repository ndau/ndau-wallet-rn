...
const mockKeyaddr = () => {
  NativeModules.KeyaddrManager = {
    KeyaddrWordsFromBytes: jest.fn((lng, ent) => {
      ...
      // not a Serious Hash™
      let hash = data.split('').reduce((a, c, i) => { a += c.charCodeAt(0); return a; }, 1);
...
const makeComponent = () => <SetupSeedPhrase navigator={navigator} parentStyles={styles} entropy={'dGVzdGluZ3dlc3Rpbmdh'} />

describe('testing SetupSeedPhrase...', () => {

  it('renders correctly', () => {
    const tree = renderer
      .create(makeComponent())
      .toJSON();
  ...
  it('calls KeyaddrWordsFromBytes as expected', () => {
    ...
    this.wrapper.update();
    expect(NativeModules.KeyaddrManager.KeyaddrWordsFromBytes.mock.calls).toEqual([['en', 'dGVzdGluZ3dlc3Rpbmdh']])
  });
});
