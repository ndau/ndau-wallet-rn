  ...
  if(result<=0){
    ...
    if(hours>=24){
     ...
     return `${Math.floor(hours)} hrs ago`;
      ...
      return `${result.toPrecision(2)} min ago`;
    }
  }
  ...
  return (
    <View style={{flex: 1, backgroundColor: '#181F28'}}>
      ...
      <Appbar.BackAction onPress={() => {navigation.goBack()}} />
        <ContentTitle title={'Latest Stats'} style={{color: 'white'}} />
      </Appbar.Header>
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            ...
            <View
              ...
              <Text style={{backgroundColor: '#012D5A', padding: 10,color:"#FFFFFF",opacity:0.5}}> BK</Text>
              <View><TouchableOpacity onPress={()=>move(item)}><Text style={{color: '#F89D1C',textDecorationLine:"underline"}}>{item.header.height}</Text><Text style={{color: 'white',opacity:0.5}}>{datecal(item.header.time)}</Text></TouchableOpacity></View>
              <Text style={{color: '#F89D1C',textDecorationLine:"underline"}}>{item.header.num_txs} txns</Text>
