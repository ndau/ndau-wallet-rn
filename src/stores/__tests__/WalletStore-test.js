...
test('Make sure we get back falsy if nothing set', async () => {
  expect(WalletStore.getWallet()).toBeFalsy()
})

test('Make sure we can set and reset a wallet', async () => {
  WalletStore.setWallet(wallet1)
  expect(WalletStore.getWallet()).toBe(wallet1)
  WalletStore.setWallet(wallet2)
  expect(WalletStore.getWallet()).toBe(wallet2)
  WalletStore.setWallet(wallet1)
  expect(WalletStore.getWallet()).toBe(wallet1)
  WalletStore.setWallet(wallet2)
  expect(WalletStore.getWallet()).toBe(wallet2)
  expect(WalletStore.getWallet()).not.toBe(wallet1)
})
